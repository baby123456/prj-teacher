...
if {$act == "rtl_chk" || $act == "sch_gen" || $act == "bhv_sim" || $act == "bit_gen"} {
	...
	if {$act == "rtl_chk" || $act == "sch_gen"} {
		...
		#add top file
		add_files ${script_dir}/../${pr_dir}/${static}.v
	...
	if {$act == "rtl_chk" || $act == "sch_gen"} {
		...
		# synthesizing design
		synth_design -mode default -top ${topmodule_src_fpga} -part ${device}
		...
		#synth user reconfigure module
		create_project -in_memory -part ${device}
		set_property source_mgmt_mode All [current_project]
		set_property board_part ${board} [current_project]
		add_files ${script_dir}/../${pr_dir}/${pm}.v
		synth_design -mode out_of_context -flatten_hierarchy rebuilt -top ${pm} -part ${device}
		write_checkpoint -force ${dcp_dir}/synth_${pm}_1.dcp
		write_checkpoint -force ${dcp_dir}/synth_${pm}_2.dcp
		write_checkpoint -force ${dcp_dir}/synth_${pm}_3.dcp
		write_checkpoint -force ${dcp_dir}/synth_${pm}_4.dcp			
		report_utilization -file ${synth_rpt_dir}/${pm}_utilization_synth.rpt
		close_project

		create_project -in_memory -part ${device}
		set_property board_part ${board} [current_project]
		add_files ${dcp_dir}/${rpt_prefix}.dcp
		add_files ${script_dir}/../sources/xdc/top_nf.xdc
		set_property USED_IN {implementation} [get_files ${script_dir}/../sources/xdc/top_nf.xdc]
		add_file ${dcp_dir}/synth_${pm}_4.dcp
		set_property SCOPED_TO_CELLS "${inst_pm4}" [get_files ${dcp_dir}/synth_${pm}_4.dcp]
		add_file ${dcp_dir}/synth_${pm}_3.dcp
		set_property SCOPED_TO_CELLS "${inst_pm3}" [get_files ${dcp_dir}/synth_${pm}_3.dcp]
		add_file ${dcp_dir}/synth_${pm}_2.dcp
		set_property SCOPED_TO_CELLS "${inst_pm2}" [get_files ${dcp_dir}/synth_${pm}_2.dcp]
		add_file ${dcp_dir}/synth_${pm}_1.dcp
		set_property SCOPED_TO_CELLS "${inst_pm1}" [get_files ${dcp_dir}/synth_${pm}_1.dcp]
		...
		# Design optimization
		opt_design
		write_checkpoint -force ${dcp_dir}/opt_design.dcp 

		# Placement
		place_design
		write_checkpoint -force ${dcp_dir}/place_design.dcp
		...
		write_checkpoint -force -cell ${inst_pm4} ${script_dir}/../shell/pr4.dcp
		# inst_user4 replace to blackbox
		update_design -cell ${inst_pm4} -black_box

		lock_design -level logical -cell ${inst_pm3}
		write_checkpoint -force -cell ${inst_pm3} ${script_dir}/../shell/pr3.dcp
		# inst_user3 replace to blackbox
		update_design -cell ${inst_pm3} -black_box

		lock_design -level logical -cell ${inst_pm2}
		write_checkpoint -force -cell ${inst_pm2} ${script_dir}/../shell/pr2.dcp
		update_design -cell ${inst_pm2} -black_box

		lock_design -level logical -cell ${inst_pm1}
		write_checkpoint -force -cell ${inst_pm1} ${script_dir}/../shell/pr1.dcp
		update_design -cell ${inst_pm1} -black_box
	
		#lock route with 4 pr be replaced with blackbox
		lock_design -level routing
		write_checkpoint -force ${script_dir}/../shell/top_static.dcp
		close_project

		open_checkpoint ${dcp_dir}/route.dcp 
		write_bitstream -force  ${out_dir}/system_full -no_partial_bitfile

		write_bitstream -force  -cell ${inst_pm4} ${out_dir}/system_pr4
		write_bitstream -force  -cell ${inst_pm3} ${out_dir}/system_pr3
		write_bitstream -force  -cell ${inst_pm2} ${out_dir}/system_pr2
		write_bitstream -force  -cell ${inst_pm1} ${out_dir}/system_pr1
